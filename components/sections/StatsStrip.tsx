...
export default function StatsStrip() {
  return (
    <section className="border-b border-primary-950/10 bg-gradient-to-r from-accent-50 via-white to-accent-50">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-6 py-12 sm:grid-cols-4 lg:px-10">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center sm:text-left">
            <p className="font-serif text-3xl text-primary-950 sm:text-4xl">{stat.value}</p>
            <p className="mt-1 text-sm leading-snug text-primary-950/70">{stat.label}</p>
          </div>
        ))}
      </div>
    </section>
